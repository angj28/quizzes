...
export default function QuizAttempt() {
  ...
  const fetchQuiz = async () => {
    try {
      ...
      setQuiz(quizData);
      setTotalPoints(questions.reduce((sum: number, q: any) => sum + q.points, 0));
    } catch (error) {
        ...
        <Col>
          <Card.Body>
            ...
            <div className="d-flex flex-wrap gap-4 align-items-center mb-3">
              <div><strong>Due</strong> {quiz?.dueDate && new Date(quiz.dueDate).toLocaleString("en-US", {
                month: "short", day: "numeric"
              })} at {quiz?.dueDate && new Date(quiz.dueDate).toLocaleString("en-US", {
              ...
              <div><strong>Available</strong> {quiz?.availableDate && new Date(quiz.availableDate).toLocaleString("en-US", {
                month: "short", day: "numeric"
              })} at {quiz?.availableDate && new Date(quiz.availableDate).toLocaleString("en-US", {
                hour: "numeric", minute: "numeric", hour12: true
              }).toLowerCase()} - {quiz?.untilDate && new Date(quiz.untilDate).toLocaleString("en-US", {
                month: "short", day: "numeric"
              })} at {quiz?.untilDate && new Date(quiz.untilDate).toLocaleString("en-US", {
                hour: "numeric", minute: "numeric", hour12: true
              }).toLowerCase()}</div>
                ...
                <table className="table table-borderless">
                  ...
                  <tbody>
                    {attempts.map((attempt: any, index: number) => (
                      <tr key={index}>
                        <td className={index === 0 ? "text-uppercase fw-bold" : ""}>
                          {index === 0 ? "Latest" : `Attempt ${index + 1}`}
                        </td>
                        <td>{attempt.timeTaken} minutes</td>
                        <td>{attempt.score} out of {totalPoints}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
